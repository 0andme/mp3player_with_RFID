...
const int resetPin = 1; // 전원 off== 태그 대기 스위치
...
void loop()
{
  ...
  checkTag(tagString, musicNOW); // 함수 - 저장된 태그 ID와 RFID 리더에서 읽은 태그 ID와 일치하는지 확인 & 처음 트랙 재생
  musicNOW++;
  clearTag(tagString);
}
...
void checkTag(char tag[], int musicNOW) {
  ...
  if (compareTag(tag, tag1)) //태그가 일치하면
  {
    ...
    while (1) // 스위치로 곡 재생 변환 권한 부여 loop
    {
      ...
      if (Next_buttonState == HIGH) //다음 곡 재생
      {
        ...
        MP3player.playTrack(musicNOW + 1);
      ...
      else if (stop_buttonState == HIGH)
      {
        if ( MP3player.getState() == playback) {
          ...
          Serial.print("음악 일시정지");
        } else if ( MP3player.getState() == paused_playback) {
          MP3player.resumeMusic();
          Serial.print("음악 다시 재생");
        }
      }
      else if (reset_buttonState == HIGH)
      { MP3player.stopTrack();
        return;
        // reset 버튼 누르면 탈출
      }
      else { //버튼 안 누르면 기다린 후 태그 기다리는 곳으로 다시 빽
        delay(500000);
        return;
      }
    }
